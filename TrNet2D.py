...
class NoBottleneck(nn.Module):
    def __init__(self, input, output):
        ...
        self.gn1 = nn.BatchNorm2d(output)
        self.conv1 = nn.Conv2d(input, output, kernel_size=3, stride=1, padding=1, dilation=1,
                               bias=False)

        self.conv1_1 = nn.Conv2d(output, output, kernel_size=1, stride=1, padding=0, bias=False)
        self.gn2 = nn.BatchNorm2d(output)
...
class DualTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        b, hwm, d = norm1.shape
        norm1 = norm1.reshape(b, H, W, d).permute(0, 3, 1, 2)

        attn = self.attn(norm1)
        b, d, h, w = attn.shape
        attn = attn.reshape(b, h*w, d)
...
class TrNet2D(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=1, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.encoder3 = nn.Conv2d(64, embed_dims[0], 3, stride=1, padding=1)
        ...
        self.decoder3 = nn.Conv2d(embed_dims[0], 64, 3, stride=1, padding=1)
        ...
        self.final = nn.Conv2d(32, num_classes, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        ### Stage 1
        out = F.relu(F.max_pool2d(self.ebn1(self.encoder1(x)), (2, 2), (2, 2)))
        # print('out', out.shape)
        out = self.layer0(out)
        # print('out', out.shape)
        t1 = out
        ### Stage 2
        out = F.relu(F.max_pool2d(self.ebn2(self.encoder2(out)), (2, 2), (2, 2)))
        out = self.layer1(out)
        t2 = out
        ### Stage 3
        out = F.relu(F.max_pool2d(self.ebn3(self.encoder3(out)), (2, 2), (2, 2)))
        out = self.layer2(out)
        t3 = out
...
if __name__ == '__main__':
    ...
    a = TrNet2D(1)
    #     # summary(a,(4,110,256,256))
    input_demo = Variable(torch.zeros(5, 1, 512, 512))
